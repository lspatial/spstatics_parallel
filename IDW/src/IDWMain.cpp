...
int main(int argc, char** argv) //输入的参数依次是：输入文件名，输出文件名，插值分辨率，采样点个数，反距离加权的幂，半径1，半径2
{
	...
	/*********定义要外部输出的变量*********/
	 ...
	 idw_rad1=std::atoi(argv[6]); //
	 idw_rad2=std::atof(argv[7]); //


        string pLayerName = argv[1];
	 string dbname=argv[8];string hostname=argv[9];string username=argv[10];string pwd=argv[11];string port=argv[12];
	...
	//idw.Read_data_sample(m_DatasourceConStr,Sample_Array);
	...
	GDALGridCreate(GGA_InverseDistanceToAPower, poOptions, sub_samplenum, Array_X, Array_Y, Array_Z,   
